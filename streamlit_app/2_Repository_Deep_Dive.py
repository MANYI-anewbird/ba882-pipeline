...
def load_repo_list():
    ...
    query = """
    SELECT DISTINCT repo_name, stars_count, cluster
...
try:
    ...
    if selected_repo:
        ...
        with col1:
            st.metric("⭐ Stars", f"{int(repo_details['stars_count'].values[0]):,}")
        with col2:
            st.metric("🍴 Forks", f"{int(repo_details['forks_count'].values[0]):,}")
        with col3:
            age_days = int(repo_details['repo_age_days'].values[0])
            st.metric("📅 Age", f"{age_days // 365} years")
        with col4:
            st.metric("💻 Commits", f"{int(repo_details['total_commits'].values[0]):,}")
        with col5:
            st.metric("👥 Contributors", f"{int(repo_details['total_contributors'].values[0]):,}")
        with col6:
            cluster = int(repo_details['cluster'].values[0])
            st.metric("🤖 ML Cluster", f"Cluster {cluster}")
