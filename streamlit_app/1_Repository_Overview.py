...
def load_cluster_data():
    """Load ML clustering results with all repo metrics"""
    query = """
    SELECT 
        repo_name,
        cluster,
...
try:
    ...
    with col1:
        st.metric("Total Repos", f"{len(df_cluster):,}")
    with col2:
        st.metric("Avg Stars", f"{df_cluster['stars_count'].mean():,.0f}")
    with col3:
        st.metric("Total Commits", f"{df_cluster['total_commits'].sum():,.0f}")
    with col4:
    ...
    df_cluster['age_years'] = df_cluster['repo_age_days'] / 365
    df_cluster['age_bin'] = pd.cut(
        df_cluster['age_years'], 
        bins=[0, 2, 5, 10, 15, 100],
        labels=['0-2 years', '2-5 years', '5-10 years', '10-15 years', '15+ years']
    )
    
    age_counts = df_cluster['age_bin'].value_counts().sort_index()
    ...
    if selected_age_bin:
        repos_in_bin = df_cluster[df_cluster['age_bin'] == selected_age_bin][
    ...
    # Prepare data
    df_fork_star['size_for_plot'] = df_fork_star['total_commits'].apply(lambda x: max(x, 100))
    
    if color_by == "Language":
        # Group less common languages
        top_languages = df_fork_star['primary_language'].value_counts().head(8).index
        df_fork_star['language_grouped'] = df_fork_star['primary_language'].apply(
    ...
    if selected_language:
        ...
        lang_repos = lang_repos.merge(
            df_cluster[['repo_name', 'stars_count', 'total_commits']],
