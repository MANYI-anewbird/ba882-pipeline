# === Function: Extract GitHub Commit Activity for each Repo ===
...
def task(request):
    """
    Cloud Function: extract latest commits for top 200 GitHub repos
    """
    ...
    for idx, repo in enumerate(repo_data[:200], start=1):
        full_name = repo.get("full_name")
        if not full_name:
            continue

        print(f"[{idx}/200] Fetching commits for {full_name}")
        ...
        try:
            ...
            if response.status_code == 403:
                ...
                print(f"Failed: {response.status_code} for {full_name}")
                continue
            ...
            print(f"✅ {len(commits)} commits from {full_name}")
    ...
    output_path = f"raw/github_commits_raw/date={run_date}/commits.json"
    ...
    return {
        "message": "GitHub commits extracted successfully!",
